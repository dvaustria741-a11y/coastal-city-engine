...
export class Activity {
  ...
  constructor(private scene:T.Scene,private collision:CollisionWorld) {
    ...
    this.boat={model:lodModel('boat'),position:new T.Vector3(143,.2,119),yaw:0,speed:0,kind:'boat'};
    for(const e of [this.car,this.boat]){e.model.position.copy(e.position);scene.add(e.model);}
    ...
    for(let i=0;i<24;i++){const p=makeHuman(colors[i%5]);this.pedestrians.push(p);scene.add(p);}
  ...
  nearest(player:Player){return [this.car,this.boat].find(e=>e.position.distanceTo(player.position)<5.5);}
  ...
  drive(dt:number,input:Input){
    ...
    if(boat){const x=e.position.x+dx,z=e.position.z+dz;if(navigableWater(x,z)&&!this.waterTraffic.some(b=>Math.hypot(b.position.x-x,b.position.z-z)<4)){e.position.x=x;e.position.z=z;}else e.speed*=.2;e.position.y=.2+Math.sin(this.elapsed*.8+e.position.x*.09)*.18;}
  ...
  update(dt:number,settings:Settings,focus:T.Vector3,camera:T.Camera) {
    ...
    for(let i=0;i<this.pedestrians.length;i++){const p=this.pedestrians[i];p.visible=i<population;if(!p.visible)continue;const u=(this.elapsed*1.2+i*18)%360,z=u<180?u-45:315-u;p.position.set(i%2?94:68,2.5,z);p.rotation.y=u<180?Math.PI:0;animateHuman(p,this.elapsed+i,1);}
