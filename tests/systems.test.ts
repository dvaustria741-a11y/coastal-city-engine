...
import {groundHeight,navigableWater,CollisionWorld,onBridge,island,BRIDGE} from '../src/world/queries';
...
describe('collision independent of render residency',()=>{
  it('supports mainland, marina, bridge and island',()=>{expect(groundHeight(101,92)).toBe(2.56);expect(groundHeight(150,110)).toBe(2.5);expect(onBridge(180,-80)).toBe(true);expect(groundHeight(180,-80)).toBe(BRIDGE.y);expect(island(282,-80)).toBe(true);expect(navigableWater(200,120)).toBe(true);expect(navigableWater(80,100)).toBe(false);});
